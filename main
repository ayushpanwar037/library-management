...
public:
    ...
    Book(int id, const string &name, const string &writer, const string &type, bool available) :
            id(id), name(name), writer(writer), type(type), available(available) {}
...
public:
    ...
    void insertBook() {
        ...
        cout << "Book Type:";
        cin >> type;
        Book newBook = Book(allBooks.size(), name, writer, type, true);
    ...
    void modifyBook() {
        ...
        cin >> option;
        if (option == 1) modify(b, "Name");
        else if (option == 2) modify(b, "Writer");
        else if (option == 3) modify(b, "Type");
    }
    ...
    void printAllBorrows() {
        if(allBorrowedBooks.size()==0){
            ...
            cout << "\tID" << "\tRoll" << "\tStudent Name" << "\tClass" << "\tBook Name" << "\tWriter Name"
                 << "\tBorrowed Date" << "\tExpected Return Date" << endl;
            cout << "---------------------------------------------------------------------------------------"
                 << "---------------------------" << endl;
